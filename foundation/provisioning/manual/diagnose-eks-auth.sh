...
echo ""
if [ -n "$IAM_ARN" ]; then
    echo "To grant access to $IAM_ARN:"
    echo ""
    echo "  ./grant-github-actions-access.sh '$IAM_ARN' '$CLUSTER_NAME'"
    echo ""
    echo "Note: This uses the modern EKS Access Entry API (recommended for EKS 1.23+)"
    echo ""
else
    echo "Run this script with the cluster name and IAM ARN to get specific fix commands:"
    echo "  ./diagnose-eks-auth.sh <CLUSTER_NAME> <IAM_ARN>"
    echo ""
    echo "Example:"
    echo "  ./diagnose-eks-auth.sh trantor arn:aws:iam::ACCOUNT:user/USERNAME"
    echo ""
fi
